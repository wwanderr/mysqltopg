...
output_dir = r'C:\Users\wcss\Desktop\mysqlToPg\create_table\migrations_ultimate'
...
for match in function_pattern.finditer(content):
    ...
    func_def = match.group(2).strip()
    functions[func_name] = f"-- ----------------------------\n-- Function structure for {func_name}\n-- ----------------------------\n{func_def}"
...
for i, (table_name, basic_def) in enumerate(tables_basic.items()):
    ...
    # 检查依赖的FUNCTION（从触发器中提取）
    used_functions = []
    if table_name in table_constraints and 'triggers' in table_constraints[table_name]:
        for trigger_def in table_constraints[table_name]['triggers']:
            # 从触发器定义中提取函数名
            func_matches = re.findall(r'EXECUTE PROCEDURE "([^"]+)"\(\)', trigger_def)
            for func_name in func_matches:
                if func_name in functions and func_name not in used_functions:
    ...
    if used_functions:
        file_content.append("-- ============================")
        file_content.append("-- FUNCTION DEFINITIONS")
        file_content.append("-- ============================")
        for func_name in used_functions:
            file_content.append(functions[func_name])
            file_content.append("")
    ...
    stats = []
    if used_functions:
        stats.append(f"函数:{len(used_functions)}")
    if used_seqs:
...
print("\n✅ 每个文件都包含完整的DDL，包括触发器函数！")
print("✅ 可直接在Navicat中执行，不会报错！")
